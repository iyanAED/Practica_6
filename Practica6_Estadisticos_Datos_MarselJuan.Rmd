...
table(Datos$ALG.cat)
t1 <- table(Datos$ALG.cat, Datos$ANM.cat)
t1
...
test_alg_anm
```
Como el p-valor es bajo (p-valor<0.05) hay suficiente evidencia estadística para rechazar la hipótesis nula en favor de la hipótesis alternativa.

```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
test_alg_sex <- chisq.test(Datos$ALG.num, Datos$Sex, correct = FALSE)

test_alg_sex
```
Como el p-valor es alto, no hay suficiente evidencia estadística para rechazar la hipótresis nula de independencia.
